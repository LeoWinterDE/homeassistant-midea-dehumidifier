...
async def async_setup_platform(hass,
    ...
    _LOGGER.info(
        "sensor.midea_dehumidifier: initializing sensor entity sub-component")
...
class MideaDehumidifierSensor(Entity):
    ...
    def __init__(self, targetDevice, hass):
        ...
        self._unique_id = 'midea_dehumidifier_' + targetDevice[
            'id'] + '_humidity'
        ...
        self._humidifier_entity_id = 'humidifier.midea_dehumidifier_' + targetDevice[
            'id']
    ...
    def __updateStateFromHumidifierEntity(self):
        ...
        if state and (ATTR_CURRRENT_HUMIDITY in state.attributes):
            ...
            _LOGGER.debug("sensor.midea_dehumidifier: current humidity = %s",
                          self._state)
        else:
            _LOGGER.debug(
                "sensor.midea_dehumidifier: cannot retrieve the state of midea_dehumidifier device"
            )
